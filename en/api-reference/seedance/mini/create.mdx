...
<Note>
This uses the same endpoint as [Create Seedance Full Video](/en/api-reference/seedance/create): `POST /api/open-api/v1/seedance/videos`. For Full (Manxue), see the full-route doc; for SP economy, see [SP Create Video](/en/api-reference/seedance/sp/create). For Official Mini, see [Create Official Mini Video](/en/api-reference/seedance/mini/create-gz).
</Note>
...
You may also pass the **billing model name** directly as `model` (e.g. `seedance2.0-mini-720p`, `seedance2.0-mini-720p-video`). In that case `resolution` is optional; the API parses it from `model`.
...
| Unit price | See the [console pricing](https://api.viraltok.ai) for the billing model name (e.g. `seedance2.0-mini-720p`) |
...
|-------|------|----------|-------------|
| `model` | string | Yes | `seedance2.0-mini` or `seedance2.0-mini-video`; billing names like `seedance2.0-mini-720p` are also accepted |
...
### Mini — billing model name (omit resolution)
...
curl -X POST 'https://api.viraltok.ai/api/open-api/v1/seedance/videos' \
  ...
  -d '{
    ...
    "prompt": "The subject slowly turns their head, breeze in the hair",
    "duration": 6,
    "ratio": "9:16",
    "images": ["https://example.com/ref-1.jpg"]
  }'
