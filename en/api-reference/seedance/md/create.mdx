...
<Note>
This endpoint is shared with [Create Seedance Video](/en/api-reference/seedance/create): `POST /api/open-api/v1/seedance/videos`. For Full (Manxue) and SP routes, see the general create doc.
</Note>
...
|-------|----------------|---------|----------|
| MD standard | `seedance2.0-md` | Per task | 1–15 s |
| MD fast | `seedance2.0-fast-md` | Per task | 1–15 s |
...
- Pass publicly reachable `https://` image or video URLs directly.
...
- At most **3** items in `reference_videos`; combined reference video **duration** must not exceed **15 seconds**, combined **size** must not exceed **200MB**; each reference video resolution must be between **720px and 2160px**.
