...
After a `seedance2.5-gz` task completes, the query response includes `data.usage`:
...
`usage` is omitted while the task is in progress or when no token data is available.
...
| `completed` | **Completed**. The video generation is complete. | `100` | The download link is available in the `result.video_url` field. For video-translate tasks with captions enabled, also see `result.caption_url`. |
